...
class MovieService {
	...
	async getMostPopularMovies() {
		const { data: movies } = await axiosClassic.get<IMovie[]>(API_URL.movies('/most-popular'));
	...
	async getBySlug(slug: string) {
		return axiosClassic.get<IMovie>(API_URL.movies('/by-slug/' + slug));
	}

	async getByActor(actorId: string) {
		return axiosClassic.get<IMovie>(API_URL.movies(`/by-actor/${actorId}`));
	}

	async getByGenres(genreIds: string[]) {
		return axiosClassic.post<IMovie[]>(API_URL.movies('/by-genres'), { genreIds });
	}
